...
ConfigLoader.prototype.verify = function(config) {
  ...
  if (!fs.existsSync(config.worker_app)) {
    console.error("Worker app path not found: %s", config.worker_app);
    return false;
  }

  if (config.stderr_path && !fs.existsSync(path.dirname(config.stderr_path))) {
    console.error("stderr path not found: %s", config.stderr_path);
    return false;
  }

  if (config.stdout_path && !fs.existsSync(path.dirname(config.stdout_path))) {
    console.error("stdout path not found: %s", config.stdout_path);
    return false;
  }

  if (config.pidfile && !fs.existsSync(path.dirname(config.pidfile))) {
    console.error("pidfile path not found: %s", config.pidfile);
    return false;
  }

  if (config.working_directory && !fs.existsSync(config.working_directory)) {
